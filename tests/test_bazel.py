...
class BazelEvidenceTest(unittest.TestCase):
    ...
    def test_decode_real_wire_fields_and_multiple_entries(self):
        ...
        # ReadRequest(1), resource_name(1); bytes_read(3) = 7.
        raw = b'\x22\x09\x2a\x07\x0a\x03\x0a\x01x\x18\x07'
    ...
    def transfer(self, direction, compressed=True, size=500, status=0):
    ...
    def test_compression_requires_matching_successful_wire_transfer(self):
        for direction in ("read", "write"):
            self.assertEqual(bazel._compressed_transfer([self.transfer(direction)], direction), [500])
